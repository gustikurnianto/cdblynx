...
let inputtulang = document.getElementById('fototulangz');

WIDTH = 800;

inputtulang.addEventListener("change", (eventtulang) => {
    let image_tulang = eventtulang.target.files[0];

    console.log("Original File", image_tulang)

    let readertulang = new FileReader();

    readertulang.readAsDataURL(image_tulang);

    readertulang.onload = (eventtulang) => {

        $('#fototulang').css('background-image', 'url('+eventtulang.target.result +')');

        image_url_tulang = eventtulang.target.result;
        let imagetulang = document.createElement('img');
        imagetulang.src = image_url_tulang;

        imagetulang.onload = (e) => {

            let canvastulang = document.createElement('canvas');
            let ratio = WIDTH / imagetulang.width;
            canvastulang.width = WIDTH;
            canvastulang.height = imagetulang.height * ratio;
            canvastulang.id = 'canvastulang';
            canvastulang.style.display = "none";

            let context = canvastulang.getContext('2d');
            context.drawImage(imagetulang, 0, 0, canvastulang.width, canvastulang.height);

            let new_image_url_tulang = canvastulang.toDataURL('image/jpeg', 98);

            // console.log("Image URL: ", new_image_url_tulang)

            let image_tulang = urlToFiletulang(new_image_url_tulang);

            var wrappertulang = document.getElementById('wrappertulang');

            wrappertulang.append(canvastulang);

        } 
    }
})

let urlToFiletulang = (url) => {
    ...
    let filetulang  = new File([dataArr], 'Fototulang.jpg', {type: mime});

    var dataTransfer = new DataTransfer();
    dataTransfer.items.add(filetulang);

    var fileInput = document.querySelector('#fototulangzx');
    fileInput.files = dataTransfer.files;

    var databaseuname = ptsft;
    var nomortulang =  document.getElementById('nomortulang').value;
    var jawabanfototulang = document.querySelector('#fototulangzx');
    var filessstulang = jawabanfototulang.files[0];
    var nameboxtulang = document.querySelector('#nameboxxtulang');
    nameboxtulang.value = databaseuname+"tulang"+nomortulang;
